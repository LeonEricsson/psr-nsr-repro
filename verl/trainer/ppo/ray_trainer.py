...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, config=None):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_psr_nsr_outcome_advantage(token_level_rewards=data.batch['token_level_rewards'],
                                                                      token_level_scores=data.batch['token_level_scores'],
                                                                      response_mask=data.batch["response_mask"],
                                                                      gamma=gamma,
                                                                      advantage="positive",
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if self.config.algorithm.adv_estimator == AdvantageEstimator.PSR_NSR and self.config.algorithm.decomposed != 'weighted':
                            if self.config.algorithm.decomposed == 'positive':
                                batch.subset(correct_idx)
                            elif self.config.algorithm.decomposed == 'negative':
